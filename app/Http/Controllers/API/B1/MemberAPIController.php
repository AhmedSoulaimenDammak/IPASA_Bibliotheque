...
class MemberAPIController extends AppBaseController
{
    ...
    {
        $member = Member::with(['address', 'membershipPlan'])->findOrFail($member->id);

        if ($member->is_default) {
            return $this->sendResponse($member->toArray(), 'Default member should not be updated.');
        }
    ...
    {
        if ($member->is_default) {
            return $this->sendResponse($member->toArray(), 'Default member should not be deleted.');
        }
