...
class AuthorAPIController extends AppBaseController
{
    ...
    public function update(Author $author, UpdateAuthorRequest $request)
    {
        if ($author->is_default) {
            return $this->sendResponse($author->toArray(), 'Default author should not be updated.');
        }
    ...
    {
        if ($author->is_default) {
            return $this->sendResponse($author->toArray(), 'Default author should not be deleted.');
        }
